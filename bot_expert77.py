...
WS_URL = "wss://game.ainm.no/ws?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..3YiZhuPlU4TrmOGCx7pmVYRhmhEKPrYaUlESr6jxd_w"
...
def decide(bot, state, ctx, path_cache):
    ...
    # RULE 5 — Idle: navigate to sector-assigned items to spread bots across the map.
    # High bot IDs (>= half) prefer the right side of the map, low IDs prefer the left.
    # This keeps idle bots out of the col-1 delivery corridor.
    if len(inv) < 3:
        n_bots     = len(state["bots"])
        prefer_right = bot_id >= (n_bots // 2)
        half_x       = width // 2
        ...
        for it in state["items"]:
            ...
            if nt is None:
                continue
            d          = mdist(pos, nt)
            in_pref    = (it["position"][0] >= half_x) == prefer_right
            if in_pref:
                if d < best_d:
