...
WS_URL = "wss://game.ainm.no/ws?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..8wGtN-PHUskEqK4LvNUr9daLWf7RhhXaBG9vBjP9tMg"
...
def next_action_toward(bot_id, pos, target, walls, width, height, avoid=None):
    ...
    # If next step is claimed by another bot, try subsequent steps in path
    if avoid:
        for step in path:
            sx, sy = step
            if (sx, sy) not in avoid:
                nx, ny = sx, sy
                break
        else:
            nx, ny = path[0]  # all claimed — take first step anyway
...
def greedy_assign(collecting_bots, remaining_types, all_items, static_walls, width, height):
    """
    Fast greedy assignment that scales to any number of bots.
    Sorts (bot, item) pairs by manhattan distance and greedily assigns nearest available.
    ...
    # Build all (distance, bot_id, item, best_adj) candidates
    candidates = []
    for bot in collecting_bots:
        ...
        for t in type_counts:
            for item in items_by_type.get(t, []):
                ...
                d = abs(best_adj[0] - bx) + abs(best_adj[1] - by)
                candidates.append((d, bot["id"], item, best_adj))
    ...
    for d, bot_id, item, best_adj in candidates:
        if bot_id in assigned_bots or item["id"] in used_items:
            continue
        t = item["type"]
        if type_remaining.get(t, 0) <= 0:
            continue
        assignments[bot_id] = (item, best_adj)
...
def deliver_toward(bot_id, pos, drop_off, walls, width, height, will_be_at=None):
    """Navigate to drop-off. Uses will_be_at to avoid already-claimed cells."""
    avoid = set(will_be_at.keys()) if will_be_at else None
    action = next_action_toward(bot_id, pos, drop_off, walls, width, height, avoid=avoid)
    return action
...
def decide(bot, state, global_remaining, claimed=None, covered_types=None,
    ...
    assignment: (item, adj_target) pre-computed by greedy_assign for this bot, or None.
    """
    if claimed is None:
        claimed = set()
    ...
    if len(inventory) >= 3:
        return deliver_toward(bot["id"], pos, drop_off, walls, width, height, will_be_at)
...
async def play():
    async with websockets.connect(WS_URL) as ws:
        ...
        while True:
            ...
            # Fast greedy assignment: scales to any number of bots
            width = state["grid"]["width"]
            height = state["grid"]["height"]
            ...
            assignments = greedy_assign(collecting_bots, list(global_remaining),
